export const cards = [
  {
    id: 1,
    title: "Website Development",
    desc: "Professional website development services to help you build, scale, and grow your online presence.",
    backGround: "#1F8447",
    img: "./img/category/website.png",
    cat: "Website Development",
  },
  {
    id: 2,
    title: "Logo Design",
    desc: "Create a unique, professional logo that makes your brand stand out from the competition.",
    backGround: "#FF7640",
    cat: "logo design",
    img: "./img/category/logo-design.png",
  },
  {
    id: 3,
    title: "Search Engine Optimization",
    desc: "Boost your website's visibility with expert SEO strategies tailored to drive traffic and improve rankings.",
    backGround: "#003912",
    cat: "SEO",
    img: "./img/category/seo.png",
  },
  {
    id: 4,
    title: "Architecture & Interior Design",
    desc: "Design your dream spaces with stunning architecture and interior design services tailored to your vision.",
    backGround: "#4D1727",
    cat: "architecture & interior designs",
    img: "./img/category/architecture.png",
  },
  {
    id: 5,
    title: "Social Media Marketing",
    desc: "Elevate your brand's online presence and engage your audience with expert social media marketing strategies.",
    backGround: "#7A831F",
    cat: "social media marketing",
    img: "./img/category/social-media-marketing_1.png",
  },
  {
    id: 6,
    title: "Voice Over",
    desc: "Bring your projects to life with professional voice over services that capture attention and convey your message.",
    backGround: "#421300",
    cat: "voice over",
    img: "./img/category/voice-over.png",
  },
  {
    id: 7,
    title: "Software Development",
    desc: "Empower your business with custom software development that enhances productivity and drives growth.",
    backGround: "#1F8447",
    cat: "software development",
    img: "./img/category/software-development.png",
  },
  {
    id: 8,
    title: "Data Science",
    desc: "Unlock valuable insights from your data with advanced data science solutions tailored to your business needs.",
    backGround: "#8F2900",
    cat: "data science",
    img: "./img/category/data-science.png",
  },
  {
    id: 9,
    title: "Product Photography",
    desc: "Showcase your products with professional photography that highlights details and enhances appeal.",
    backGround: "#687200",
    cat: "photography",
    img: "./img/category/product-photography.png",
  },
  {
    id: 10,
    title: "Ecommerce Marketing",
    desc: "Boost your online store's success with ecommerce marketing strategies designed to drive sales and customer engagement.",
    backGround: "#00732E",
    cat: "ecommerce marketing",
    img: "./img/category/e-commerce.png",
  },
  {
    id: 11,
    title: "Video Editing",
    desc: "Transform your raw footage into captivating videos with expert editing services that bring your vision to life.",
    backGround: "#BE5272",
    cat: "video editing",
    img: "./img/category/video-editing.png",
  },
];

export const helperCatgory = [
  {
    icon: "./img/helpercategory/programming.svg",
    title: "Programming & Tech",
  },
  {
    icon: "./img/helpercategory/graphics.svg",
    title: "Graphics & Design",
  },
  {
    icon: "./img/helpercategory/digital-marketing.svg",
    title: "Digital Marketing",
  },
  {
    icon: "./img/helpercategory/writing.svg",
    title: "Writing & Translation",
  },
  {
    icon: "./img/helpercategory/video.svg",
    title: "Videos & Animation",
  },
  {
    icon: "./img/helpercategory/AI.svg",
    title: "AI Services",
  },
  {
    icon: "./img/helpercategory/music.svg",
    title: "Music & Audio",
  },
  {
    icon: "./img/helpercategory/bussniess.svg",
    title: "Bussniess ",
  },
  {
    icon: "./img/helpercategory/consulting.svg",
    title: "Consulting",
  },
];

export const projects = [
  {
    id: 1,
    img: "https://images.pexels.com/photos/1462935/pexels-photo-1462935.jpeg?auto=compress&cs=tinysrgb&w=1600",
    pp: "https://images.pexels.com/photos/774909/pexels-photo-774909.jpeg?auto=compress&cs=tinysrgb&w=1600",
    cat: "Web and Mobile Design",
    username: "Anna Bell",
  },
  {
    id: 2,
    img: "https://images.pexels.com/photos/270408/pexels-photo-270408.jpeg?auto=compress&cs=tinysrgb&w=1600",
    pp: "https://images.pexels.com/photos/91227/pexels-photo-91227.jpeg?auto=compress&cs=tinysrgb&w=1600",
    cat: "Logo Design",
    username: "Morton Green",
  },
  {
    id: 3,
    img: "https://images.pexels.com/photos/4144923/pexels-photo-4144923.jpeg?auto=compress&cs=tinysrgb&w=1600",
    pp: "https://images.pexels.com/photos/1181686/pexels-photo-1181686.jpeg?auto=compress&cs=tinysrgb&w=1600",
    cat: "Animated GIFs",
    username: "Emmett Potter",
  },
  {
    id: 4,
    img: "https://images.pexels.com/photos/4348404/pexels-photo-4348404.jpeg?auto=compress&cs=tinysrgb&w=1600",
    pp: "https://images.pexels.com/photos/2182970/pexels-photo-2182970.jpeg?auto=compress&cs=tinysrgb&w=1600",
    cat: "Packaging Design",
    username: "Freddie Johnston",
  },
  {
    id: 5,
    img: "https://images.pexels.com/photos/4458554/pexels-photo-4458554.jpeg?auto=compress&cs=tinysrgb&w=1600",
    pp: "https://images.pexels.com/photos/733872/pexels-photo-733872.jpeg?auto=compress&cs=tinysrgb&w=1600",
    cat: "Social Media Design",
    username: "Audrey Richards",
  },
  {
    id: 6,
    img: "https://images.pexels.com/photos/4465831/pexels-photo-4465831.jpeg?auto=compress&cs=tinysrgb&w=1600",
    pp: "https://images.pexels.com/photos/1036627/pexels-photo-1036627.jpeg?auto=compress&cs=tinysrgb&w=1600",
    cat: "Illustration",
    username: "Dalton Hudson",
  },
  {
    id: 7,
    img: "https://images.pexels.com/photos/6077368/pexels-photo-6077368.jpeg?auto=compress&cs=tinysrgb&w=1600",
    pp: "https://images.pexels.com/photos/1858175/pexels-photo-1858175.jpeg?auto=compress&cs=tinysrgb&w=1600",
    cat: "Book Design",
    username: "Hannah Dougherty",
  },
  {
    id: 8,
    img: "https://images.pexels.com/photos/4065876/pexels-photo-4065876.jpeg?auto=compress&cs=tinysrgb&w=1600",
    pp: "https://images.pexels.com/photos/1680175/pexels-photo-1680175.jpeg?auto=compress&cs=tinysrgb&w=1600",
    cat: "Digital Marketing",
    username: "Ward Brewer",
  },
];

export const gigs = [
  {
    id: 1,
    images: [
      {
        img: "https://images.pexels.com/photos/580151/pexels-photo-580151.jpeg?auto=compress&cs=tinysrgb&w=1600",
      },
      {
        img: "https://images.pexels.com/photos/1462935/pexels-photo-1462935.jpeg?auto=compress&cs=tinysrgb&w=1600",
      },
    ],
    pp: "https://images.pexels.com/photos/720598/pexels-photo-720598.jpeg?auto=compress&cs=tinysrgb&w=1600",
    desc: "I will create ai art character from your images and prompts",
    price: 59,
    star: 5,
    username: "Anna Bell",
  },
  {
    id: 2,
    images: [
      {
        img: "https://images.pexels.com/photos/1462935/pexels-photo-1462935.jpeg?auto=compress&cs=tinysrgb&w=1600",
      },
      {
        img: "https://images.pexels.com/photos/4145190/pexels-photo-4145190.jpeg?auto=compress&cs=tinysrgb&w=1600",
      },
    ],

    pp: "https://images.pexels.com/photos/1036627/pexels-photo-1036627.jpeg?auto=compress&cs=tinysrgb&w=1600",
    desc: "I will create ultra high quality character art with ai",
    price: 79,
    star: 5,
    username: "Lannie Coleman",
  },
  {
    id: 3,
    images: [
      {
        img: "https://images.pexels.com/photos/1462935/pexels-photo-1462935.jpeg?auto=compress&cs=tinysrgb&w=1600",
      },
      {
        img: "https://images.pexels.com/photos/8797307/pexels-photo-8797307.jpeg?auto=compress&cs=tinysrgb&w=1600",
      },
    ],
    pp: "https://images.pexels.com/photos/1062280/pexels-photo-1062280.jpeg?auto=compress&cs=tinysrgb&w=1600",
    desc: "I will creating unique ai generated artworks mid journey ai artist",
    price: 112,
    star: 5,
    username: "Carol Steve",
  },
  {
    id: 4,
    images: [
      {
        img: "https://images.pexels.com/photos/1462935/pexels-photo-1462935.jpeg?auto=compress&cs=tinysrgb&w=1600",
      },
      {
        img: "https://images.pexels.com/photos/5708069/pexels-photo-5708069.jpeg?auto=compress&cs=tinysrgb&w=1600",
      },
    ],
    pp: "https://images.pexels.com/photos/1681010/pexels-photo-1681010.jpeg?auto=compress&cs=tinysrgb&w=1600",
    desc: "I will create custom ai generated artwork using your photos",
    price: 99,
    star: 4,
    username: "Don Weber",
  },
  {
    id: 5,
    images: [
      {
        img: "https://images.pexels.com/photos/1462935/pexels-photo-1462935.jpeg?auto=compress&cs=tinysrgb&w=1600",
      },
      {
        img: "https://images.pexels.com/photos/5699456/pexels-photo-5699456.jpeg?auto=compress&cs=tinysrgb&w=1600",
      },
    ],
    pp: "https://images.pexels.com/photos/1771383/pexels-photo-1771383.jpeg?auto=compress&cs=tinysrgb&w=1600",
    desc: "I will recreate your dreams in high quality pictures",
    price: 59,
    star: 5,
    username: "Audrey Richards",
  },
  {
    id: 6,
    images: [
      {
        img: "https://images.pexels.com/photos/1462935/pexels-photo-1462935.jpeg?auto=compress&cs=tinysrgb&w=1600",
      },
      {
        img: "https://images.pexels.com/photos/8100784/pexels-photo-8100784.jpeg?auto=compress&cs=tinysrgb&w=1600",
      },
    ],
    pp: "https://images.pexels.com/photos/715546/pexels-photo-715546.jpeg?auto=compress&cs=tinysrgb&w=1600",
    desc: "I will create ai digital art illustration hyper realistic painting",
    price: 79,
    star: 4,
    username: "Walton Shepard ",
  },
  {
    id: 7,
    images: [
      {
        img: "https://images.pexels.com/photos/1462935/pexels-photo-1462935.jpeg?auto=compress&cs=tinysrgb&w=1600",
      },
      {
        img: "https://images.pexels.com/photos/6039245/pexels-photo-6039245.jpeg?auto=compress&cs=tinysrgb&w=1600",
      },
    ],
    pp: "https://images.pexels.com/photos/720606/pexels-photo-720606.jpeg?auto=compress&cs=tinysrgb&w=1600",
    desc: "I will generate images with your prompts using ai dalle",
    price: 89,
    star: 5,
    username: "Waverly Schaefer",
  },
  {
    id: 8,
    images: [
      {
        img: "https://images.pexels.com/photos/1462935/pexels-photo-1462935.jpeg?auto=compress&cs=tinysrgb&w=1600",
      },
      {
        img: "https://images.pexels.com/photos/5490778/pexels-photo-5490778.jpeg?auto=compress&cs=tinysrgb&w=1600",
      },
    ],
    pp: "https://images.pexels.com/photos/1699159/pexels-photo-1699159.jpeg?auto=compress&cs=tinysrgb&w=1600",
    desc: "I will create custom art using midjourney generator",
    price: 110,
    star: 4,
    username: "Wilton Hunt",
  },
];

export const reviews = [
  {
    id: "1",
    user: {
      username: "Lisbon",
      profileImage:
        "https://fiverr-res.cloudinary.com/image/upload/f_auto,q_auto,t_profile_small/v1/attachments/profile/photo/13d3ee6c4d6d6c7829af9acb9ce1302c-1684766510579/7357fb1e-dd09-4fdd-b1de-74cc7a433520.jpg",
      country: "Spain",
    },
    comment:
      "Super fast he was to deliver, that was second experience and would work again for sure",
    projectPrice: "PKR14,000-PKR28,000",
    duration: "1",
    rating: "4",
  },
  {
    id: "2",
    user: {
      username: "Bond",
      profileImage:
        "https://fiverr-res.cloudinary.com/image/upload/f_auto,q_auto,t_profile_small/v1/attachments/profile/photo/13d3ee6c4d6d6c7829af9acb9ce1302c-1684766510579/7357fb1e-dd09-4fdd-b1de-74cc7a433520.jpg",
      country: "itley",
    },
    comment:
      "Never disappoints, super professional and creative Best found here",
    projectPrice: "PKR14,000-PKR28,000",
    duration: "2",
    rating: "4",
  },
  {
    id: "3",
    user: {
      username: "Hassan",
      profileImage:
        "https://fiverr-res.cloudinary.com/image/upload/f_auto,q_auto,t_profile_small/v1/attachments/profile/photo/13d3ee6c4d6d6c7829af9acb9ce1302c-1684766510579/7357fb1e-dd09-4fdd-b1de-74cc7a433520.jpg",
      country: "Spain",
    },
    comment:
      "Super fast he was to deliver, that was second experience and would work again for sure",
    projectPrice: "PKR14,000-PKR28,000",
    duration: "1",
    rating: "4",
  },
  {
    id: "4",
    user: {
      username: "Shahzaib",
      profileImage:
        "https://fiverr-res.cloudinary.com/image/upload/f_auto,q_auto,t_profile_small/v1/attachments/profile/photo/13d3ee6c4d6d6c7829af9acb9ce1302c-1684766510579/7357fb1e-dd09-4fdd-b1de-74cc7a433520.jpg",
      country: "pakistan",
    },
    comment:
      "Super fast he was to deliver, that was second experience and would work again for sure",
    projectPrice: "PKR14,000-PKR28,000",
    duration: "1",
    rating: "4",
  },
  {
    id: "5",
    user: {
      username: "Hamza",
      profileImage:
        "https://fiverr-res.cloudinary.com/image/upload/f_auto,q_auto,t_profile_small/v1/attachments/profile/photo/13d3ee6c4d6d6c7829af9acb9ce1302c-1684766510579/7357fb1e-dd09-4fdd-b1de-74cc7a433520.jpg",
      country: "Pakistan",
    },
    comment:
      "Super fast he was to deliver, that was second experience and would work again for sure",
    projectPrice: "PKR14,000-PKR28,000",
    duration: "1",
    rating: "4",
  },
  {
    id: "6",
    user: {
      username: "Khurum",
      profileImage:
        "https://fiverr-res.cloudinary.com/image/upload/f_auto,q_auto,t_profile_small/v1/attachments/profile/photo/13d3ee6c4d6d6c7829af9acb9ce1302c-1684766510579/7357fb1e-dd09-4fdd-b1de-74cc7a433520.jpg",
      country: "Pakistan",
    },
    comment:
      "Super fast he was to deliver, that was second experience and would work again for sure",
    projectPrice: "PKR14,000-PKR28,000",
    duration: "1",
    rating: "4",
  },
  {
    id: "7",
    user: {
      username: "Umar",
      profileImage:
        "https://fiverr-res.cloudinary.com/image/upload/f_auto,q_auto,t_profile_small/v1/attachments/profile/photo/13d3ee6c4d6d6c7829af9acb9ce1302c-1684766510579/7357fb1e-dd09-4fdd-b1de-74cc7a433520.jpg",
      country: "Pakistan",
    },
    comment:
      "Super fast he was to deliver, that was second experience and would work again for sure",
    projectPrice: "PKR14,000-PKR28,000",
    duration: "1",
    rating: "4",
  },
  {
    id: "8",
    user: {
      username: "Tom",
      profileImage:
        "https://fiverr-res.cloudinary.com/image/upload/f_auto,q_auto,t_profile_small/v1/attachments/profile/photo/13d3ee6c4d6d6c7829af9acb9ce1302c-1684766510579/7357fb1e-dd09-4fdd-b1de-74cc7a433520.jpg",
      country: "USA",
    },
    comment:
      "Super fast he was to deliver, that was second experience and would work again for sure",
    projectPrice: "PKR14,000-PKR28,000",
    duration: "1",
    rating: "4",
  },
  {
    id: "9",
    user: {
      username: "Lisbon",
      profileImage:
        "https://fiverr-res.cloudinary.com/image/upload/f_auto,q_auto,t_profile_small/v1/attachments/profile/photo/13d3ee6c4d6d6c7829af9acb9ce1302c-1684766510579/7357fb1e-dd09-4fdd-b1de-74cc7a433520.jpg",
      country: "Spain",
    },
    comment:
      "Super fast he was to deliver, that was second experience and would work again for sure",
    projectPrice: "PKR14,000-PKR28,000",
    duration: "1",
    rating: "4",
  },
  {
    id: "10",
    user: {
      username: "Lisbon",
      profileImage:
        "https://fiverr-res.cloudinary.com/image/upload/f_auto,q_auto,t_profile_small/v1/attachments/profile/photo/13d3ee6c4d6d6c7829af9acb9ce1302c-1684766510579/7357fb1e-dd09-4fdd-b1de-74cc7a433520.jpg",
      country: "Spain",
    },
    comment:
      "Super fast he was to deliver, that was second experience and would work again for sure",
    projectPrice: "PKR14,000-PKR28,000",
    duration: "1",
    rating: "4",
  },
];

export const footer = [
  {
    title: "Categories",
    links: [
      {
        name: "Graphics & Design",
        links: "/",
      },
      {
        name: "Digital Marketing",
        links: "/",
      },
      {
        name: "Writing & Translation",
        links: "/",
      },
      {
        name: "Video & Animation",
        links: "/",
      },
      {
        name: "Music & Audio",
        links: "/",
      },
      {
        name: "Fiverr Logo Maker",
        links: "/",
      },
      {
        name: "Programming & Tech",
        links: "/",
      },
      {
        name: "Data",
        links: "/",
      },
      {
        name: "Business",
        links: "/",
      },
      {
        name: "Personal Growth & Hobbies",
        links: "/",
      },
      {
        name: "Photography",
        links: "/",
      },
      {
        name: "Finance",
        links: "/",
      },
      {
        name: "End-to-End Projects",
        links: "/",
      },
    ],
  },
  {
    title: "About",
    links: [
      {
        name: "Careers",
        link: "/",
      },
      {
        name: "Press & News",
        link: "/",
      },
      {
        name: "Partnership",
        link: "/",
      },
      {
        name: "Privacy & Policy",
        link: "/",
      },
      {
        name: "Terms of service",
        link: "/",
      },
      {
        name: "Intellectual Property Claims",
        link: "/",
      },
      {
        name: "Investors",
        link: "/",
      },
    ],
  },
  {
    title: "Support & Education",
    links: [
      {
        name: "Help & Support",
        link: "/",
      },
      {
        name: "Trust & saftey",
        link: "/",
      },
      {
        name: "Quality Guide",
        link: "/",
      },
      {
        name: "Selling on Fiverr",
        link: "/",
      },
      {
        name: "Buying on Fiverr",
        link: "/",
      },
      {
        name: "Fiverr Guides",
        link: "/",
      },
      {
        name: "Learn",
        link: "/",
      },
    ],
  },
  {
    title: "Community ",
    links: [
      {
        name: "Customer Success Stories",
        link: "/",
      },
      {
        name: "Community Hub",
        link: "/",
      },
      {
        name: "Forum",
        link: "/",
      },
      {
        name: "Events ",
        link: "/",
      },
      {
        name: "Belogs ",
        link: "/",
      },
      {
        name: "Creators ",
        link: "/",
      },
      {
        name: "Affiliates",
        link: "/",
      },
      {
        name: "Podcast",
        link: "/",
      },
      {
        name: "Invite a Friend",
        link: "/",
      },
      {
        name: "Become a Seller ",
        link: "/",
      },
      {
        name: "Community Standard ",
        link: "/",
      },
    ],
  },
  {
    title: "Business Solutions ",
    links: [
      {
        name: "About Business Solutions",
        link: "/",
      },
      {
        name: "Fiverr Pro ",
        link: "/",
      },
      {
        name: "Fiverr Certified",
        link: "/",
      },
      {
        name: "Become an Agency ",
        link: "/",
      },
      {
        name: "Fiverr Enterprise ",
        link: "/",
      },
      {
        name: "ClearVoice ",
        link: "/",
      },
      {
        name: "Working Not Working",
        link: "/",
      },
      {
        name: "Contact Sales",
        link: "/",
      },
    ],
  },
];
